...
const SYSTEM_PROMPT =
  ...
  "own AI assistant, running on Groq and Google Gemini — never claim to be " +
  "ChatGPT, GPT-4, or any other OpenAI product, and don't cite a training cutoff " +
  "date as if you were one of those products.";
...
export function buildSystemInstruction({ savedMemories, allowMemorySave, searchResults } = {}) {
...
async function askGroqConversation(history, memoryOptions, { thinking = false } = {}) {
  const body = {
    model: GROQ_MODEL,
    messages: [
...
export async function getConversationReply(history, memoryOptions, opts = {}) {
  try {
    return await askGroqConversation(history, memoryOptions, opts);
  } catch (err) {
